...
const Brands = () => {
  ...
  // Xóa thương hiệu (confirm modal)
  const openDeleteConfirm = (id, name) => setDeleteConfirm({ isOpen: true, id, name });

  const confirmDelete = async () => {
    const brandId = deleteConfirm.id;
    if (!brandId) return;
    try {
      setDeleting(true);
      const response = await fetch(
  ...
  const cancelDelete = () => setDeleteConfirm({ isOpen: false, id: null, name: "" });
    ...
    <Container>
      <div className="space-y-6">
              ...
              >
                <div className="p-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => openDeleteConfirm(brand._id, brand.name)}
        ...
        <ConfirmModal
          ...
          message={`Bạn có chắc muốn xóa thương hiệu "${deleteConfirm.name}"? Hành động này không thể hoàn tác.`}
